...
contract ArcadeToken {
    using SafeMath for uint;              // initiate SafeMath 
    ...
    function spend(address payable recipient) public payable {
        uint fee = msg.value.mul(fee_rate).div(10000);                // calculate 0.25% from basis points using msg.value (percent = basis_points * your_number / 10000)
        uint reward = msg.value.mul(reward_rate);                    // reward 3 points for every wei spent (multiply msg.value)
